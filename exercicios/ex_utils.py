...
def propose_exercize(nq, nd):
    ...
    nq=len(Q)
    nd=len(Q[0])
    print('Seu sistema de IR recebeu %d consultas dos usuários'%nq)
    print('Aqui estão os top-%d resulados que seu sistema retornou para cada consulta, '%nd+\
          'e a avaliação correta de cada um deles')
...
def solve(ex,Q,R):
    ...
    def compute_PR(print_screen=True):
        ...
        Rec_tot=[]        
        if print_screen:
            print('Precisão e Recall em k para k=1,...,%d'%nd)      
        for q in range(nq):
            q1=q+1
            r=R_[q,:]
            if print_screen:
    ...
    def compute_TPFP(TP_rate=None):
        TP_tot=[]        
        FP_tot=[]        
        print('TP_rate e FP_rate em k para k=1,...,%d'%nd)      
        for q in range(nq):
            q1=q+1
            r=R_[q,:]
            nr=1-r
    ...
    if ex=='prec_rec' or ex=='all':        
        ...
        for q in range(nq):
            ...
            plt.plot(np.array(Rec_q), np.array(Prec_q),label='Curva Precision-Recall')            
            ...
            plt.plot(R_int,P_int,color='r',label='Curva PR interpolada')
    ...
    if ex=='r-prec' or ex=='all':        
        ...
        for q in range(nq):            
            ...
            q1=q+1
            print('\tConsulta %d'%q1)
            print('\t\tNúmero de documentos relevantes: %d --> P(%d)=%.2f'%(r,r,Prec_q[r-1]))
    if ex=='map' or ex=='all':        
        if Prec_tot==[]:
            Prec_tot, Rec_tot=compute_PR()
    ...
    if ex=='roc' or ex=='all' or ex=='auc':
        ...
        for q in range(nq):
            ...
            plt.plot(np.array(FP_q_), np.array(TP_q_),label='Curva ROC')            
            plt.xlim([-0.05,1.05]); plt.ylim([-0.05,1.05])
