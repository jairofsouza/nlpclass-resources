...
def propose_exercize(doc=-1, quer=-1):
    ...
    print('Dada a consulta')
    print('\t * %s'%q)
    print('E uma coleção de documentos')
    for d in D:
        print('\t * %s'%d)
    model='freq'
    if model=='freq':
        print('E um modelo de frequência de termos')
        print('\t * tf_{i,j}=freq_{i,j}')
    print('Determinar o ranking da coleção de '+\
          'documentos em relação a uma dada consulta utilizando o'+\
          ' Vector Space Model com as seguintes medidas de similaridade:')
...
def define_idf(words,V, M,N):
    ...
    print()
    print('Frequência inversa do termo (IDF)')
    print('t_i\t\tn_i\tidf_i')    
...
def define_tf(V, words, query=False):
    ...
    print()
    if query:
        print('Frequência do termo na consulta (TF)')
        print('t_i\t\ttf_{i,q}')        
    else:
        print('Frequência do termo (TF)')
