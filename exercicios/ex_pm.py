...
def propose_exercize(N=4, M=6, random=True):
    ...
    print('Seja')
    print('\tVetor de incidência da consulta')
    print('\t\tq=%s'%str(Q[0]))
    print('Vetores de incidência dos documentos')
    for i, d in enumerate(D):
        print('\t\td%d=%s'%(i+1,str(d)))
    print('\tInicialize')
    print('\t\tp_i=0.5')
    print('Considere como relevante os top-%d documentos'%2)
    return D,Q

def reduce_terms(D,Q):
    print('Reduzir os vetores dos documentos considerando apenas os termos que aparecem na consulta')
...
def update(D,R):
    ...
    print('\nDocumentos relevantes: {d%d, d%d}'%(rel[0]+1,rel[1]+1))
